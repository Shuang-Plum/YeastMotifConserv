...
if (refstr=='S288C') {refstr<-'STC'}
  ...
  if (length(sm.res)>0) {
    ...
    bkg.dist<-read.csv('./Data/BLOSUM62_dist.csv',header=T)
    submat<-read.csv('./Data/BLOSUM62_KarlinNorm.csv',header=T,row.names = 'AAS')
    tree<-read.csv('./Data/UPGMA_nodes.csv',row.names = 'X')
