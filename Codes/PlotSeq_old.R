...
SeqPos<-function(gene,refstr,sm.res,co_filep) {
  ...
  numplot$num<-as.character(c(1:seqlen))
  
  # get the match and highlight in yellow
  # get the col of refstr match character
  m.label<-sm.res[,which(colnames(sm.res)==refstr)]
  sm.uni.row<-!duplicated(sm.res$position)
  m.label<-m.label[sm.uni.row]
  m.xpos<-xpos[sm.res$position[sm.uni.row]]
  m.ypos<-ypos[sm.res$position[sm.uni.row]]
  ...
  sm.res.list<-list(sm.res,seqplot,hlplot,numplot,xmax,ymax)
...
PlotSeq<-function(sm.res.list) {
  seqplot<-sm.res.list()$seqplot
  hlplot<-sm.res.list()$hlplot
  numplot<-sm.res.list()$numplot
  xmax<-sm.res.list()$xmax
  ymax<-sm.res.list()$ymax
